...
class Scraper(object):
		...
		def avechiScrapper(self,x,y):
			...
			try:
				...
				if soup.find('div',{"class":"message notice"}):
					...
					discount=0
					print(price)
				# cur.execute('INSERT INTO kenyan_stores_scraper_avechi(product_price,product_warranty,product_discount,product_seller,product_id_id,retureturn_time,timestamp) VALUES(%s,%s,%s,%s,%s,%s,%s)',(price,warranty,discount,vendor,id,str(return_time),time_now))
				# conn.commit()
		...
		def JumiaScraper(self,x,y):
			try:
				...
				# cur.execute('INSERT INTO kenyan_stores_scraper_jumia(product_price,product_warranty,product_discount,product_seller,product_id_id,return_time,timestamp) VALUES(%s,%s,%s,%s,%s,%s,%s)',(price_jumia,warranty,discount,seller,id,return_time,time_now))
				# cur.execute('update kenyan_stores_scraper_products set product_key_features=productDetails where id = %s',id)
				# conn.commit()
		...
		def killmallScraper(self,x,y):
			...
			try:
				...
				if soup.find('div',{"class":"search-empty"}):
					...
					seller= ''
					discount=0
				# cur.execute('INSERT INTO kenyan_stores_scraper_jumia(product_price,product_warranty,product_discount,product_seller,product_id_id,return_time,timestamp) VALUES(%s,%s,%s,%s,%s,%s,%s)',(price,warranty,discount,seller,id,return_time,time_now))
				# conn.commit()
